...
| `PRODUCTION_SITE_URL` | `https://dondeaprendoaws.com` in this repository; set only after public DNS cutover is healthy in a new deployment |

Set them in the repository's **Settings → Secrets and variables → Actions → Variables** page. These values identify the AWS deployment; the role ARN is not a secret. The current repository has `PRODUCTION_SITE_URL` set, so both the apex and preview are verified after every publish and artifact restore. In a fresh deployment, keep it unset until the production DNS cutover is healthy. The publisher requires this value to be the apex HTTPS origin, without a path, query, or fragment.
...
For the production domain, first create the Route 53 public hosted zone and copy the existing DNS records, including the old apex and `www` A records. Compare the new zone's answers, then change the registrar nameservers to delegate to Route 53 while the old site still serves web traffic. Next, update the stack with the hosted zone ID to request the certificate and attach the apex and `www` aliases. Wait for certificate validation and CloudFront deployment, then preflight HTTPS using the production Host header and matching TLS SNI while the old A records remain. Only after that succeeds, replace the website A/AAAA records with CloudFront aliases and verify the public site. Set `PRODUCTION_SITE_URL` after the production cutover is healthy.

Find the Route 53 public hosted zone ID under **Hosted zones**. For `dondeaprendoaws.com`, the current ID is `Z06015882Q8LKJ01VFCP9`; pass the ID without a `/hostedzone/` prefix. It is an identifier, not a secret.

```sh
aws sts get-caller-identity --region us-east-1
CHANGE_SET_TYPE=UPDATE # The dondeaprendoaws stack already exists.
...
aws cloudformation create-change-set \
  ...
  --parameters \
    ...
    ParameterKey=HostedZoneId,ParameterValue=Z06015882Q8LKJ01VFCP9 \
...
Read `SiteBucketName`, `DistributionId`, `DistributionDomainName`, `CertificateArn`, and `PublisherRoleArn` from the stack outputs and set the repository variables above. Keep the bucket and its publication when deleting or replacing the stack; CloudFormation is configured to retain the bucket. The preview uses the CloudFront hostname and remains noindex. A viewer-request CloudFront Function redirects `www` requests to the apex and maps `/blog/` and `/blog/<slug>/` to their generated `index.html` objects in private S3. It redirects the observed slashless variants to trailing-slash URLs while preserving query strings; direct file and asset paths bypass only the blog rewrite. The function is part of the manually managed stack and is not applied by GitHub Actions. The production domain and certificate are configured in AWS account `719535286359`.

## DNS recovery during the initial cutover

The old platform remains available while DNS caches expire. If the CloudFront site fails during this initial window, change only the website records in Route 53 hosted zone `Z06015882Q8LKJ01VFCP9`: restore the apex A record to `44.218.84.107` with TTL 600 and the `www` A record to the same address with TTL 3600, then delete the CloudFront alias AAAA records for both names. Leave MX, TXT, certificate-validation CNAMEs, nameservers, and registrar delegation untouched. Wait for the Route 53 change to become `INSYNC`, then verify the old site through both hostnames. This recovery target is valid only while the old platform remains active; do not use the saved IP after retiring it. For a bad content publish with healthy routing, use the build-artifact restore workflow instead of changing DNS.
