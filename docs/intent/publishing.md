...
The repository is the source of truth. Hugo builds `public/`; pull requests run the same local checks without AWS credentials. Each push to `main` saves that exact build, assumes a short-lived AWS role through GitHub OIDC, publishes it to the private S3 bucket, invalidates CloudFront, waits for completion, and checks the CloudFront preview's homepage, stylesheet, noindex 404, and exact `/revision.json` over HTTPS. Once `PRODUCTION_SITE_URL` is set after public cutover, the same publish also checks the apex HTTPS homepage, indexability, apex canonical URL, noindex 404, and exact revision. Redirects to another host fail verification.
...
| `PRODUCTION_SITE_URL` | Optional; set to `https://dondeaprendoaws.com` only after public DNS cutover is healthy |

Set them in the repository's **Settings → Secrets and variables → Actions → Variables** page. These values identify the AWS deployment; the role ARN is not a secret. Keep `PRODUCTION_SITE_URL` unset while validating the CloudFront preview and before the production DNS cutover. After the apex serves the replacement successfully, set it so subsequent publishes and artifact restores also verify both hosts. The publisher requires this value to be the apex HTTPS origin, without a path, query, or fragment.
