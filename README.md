...
The AWS preview serves the resource directory and blog. The existing production domain remains on its current platform until the domain cutover. The migration hostname is excluded from indexing and does not send visits to Google Analytics. Content submissions and SEObot are not part of this site.
